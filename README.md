...
- Keep `.streamlit/config.toml` committed so Streamlit uses the intended runtime settings.
- Do not commit the `.venv/` directory.
- Keep `requirements.txt` and `requirements-dev.txt` synced with the environment.
...
3. Confirm the repository contains only source files and ignore files, not the `.venv/` directory.
